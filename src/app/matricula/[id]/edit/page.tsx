...
export default function EditEnrollmentPage() {
  ...
  const convertFirestoreToFormData = (data: FirestoreEnrollment): Partial<EnrollmentFormData> => {
    return {
      step1: {
        ...
        gemeos: data.gemeos,
        temIrmaosNaCreche: data.temIrmaosNaCreche,
      ...
      step2: {
        guardians: [] // Será carregado das subcoleções
      },
      step3: {
      ...
      step5: {
        members: [] // Será carregado das subcoleções
      },
      step6: {
        serie: data.serie,
        anoLetivo: data.anoLetivo,
        authorizedPersons: [] // Será carregado das subcoleções
      },
      step7: {
  ...
  const handleComplete = async (data: EnrollmentFormData) => {
    ...
    try {
      await EnrollmentService.updateEnrollment(enrollmentId, data, user.uid)
